...
- Финальная DNN: **input → 64 → 32 → 16 → 1**, Adam, `lr=0.001`.
...
Для табличных данных была реализована отдельная `DNN` на PyTorch. В ходе экспериментов изменялись количество скрытых слоёв, их ширина, `dropout`, оптимизатор и `learning_rate`. Лучшей оказалась архитектура `input(26) → 64 → 32 → 16 → 1` с оптимизатором Adam и `learning_rate=0.001`.
